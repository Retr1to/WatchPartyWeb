...
export class SocketService {
  ...
  private handleMessage(data: string): void {
    try {
      ...
      switch (message.type) {
        ...
        case 'room_state':
          ...
          try {
            ...
            if (roomData.users && Array.isArray(roomData.users)) {
              ...
              // Emitir el estado completo de la sala
        ...
        case 'video_uploaded':
          this.videoChangedSubject.next({ url: message.videoFileName || '' });
          break;
  ...
  // ============================================================
  // MÉTODOS PÚBLICOS - Compatible con el código existente
  // ============================================================

  async createRoom(username: string): Promise<void> {
    // Generar roomId y userId
    ...
    try {
      ...
      // Simular respuesta de room created
      const room: Room = {
  ...
  async joinRoom(roomCode: string, username: string): Promise<void> {
    ...
    try {
      ...
      // Simular respuesta de room joined
      const room: Room = {
        code: roomCode,
        host: 'unknown', // El backend enviará el estado real
        participants: [{
          id: this.currentUserId,
          username: username,
          isHost: false
  ...
  changeVideo(roomCode: string, url: string): void {
    // Este método se usaría para cambiar videos subidos
  ...
  // ============================================================
  // OBSERVABLES - Mantienen compatibilidad con código existente
  // ============================================================
